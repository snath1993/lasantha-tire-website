        ...
        const data = {
            ...
            items: rows.map((row, index) => {
                ...
                return {
                    ...
                    size: size || '',
                    country: mapping.COUNTRY ? safeText(row[mapping.COUNTRY], '') : '',
                    itemClass: row.ItemClass != null ? toNum(row.ItemClass) : null,
                    foc: row.FOC != null ? toNum(row.FOC) : 0,
                    wac: safeText(row.WAC, ''),
                    wbc: safeText(row.WBC, ''),
                    n2Rate: safeText(row.N2Rate, ''),
                    plyRate: row.PLYRate != null ? safeText(row.PLYRate, '') : '',
                    yom: row.YOM != null ? safeText(row.YOM, '') : '',
                    lsSymbol: row.LSSymbol != null ? safeText(row.LSSymbol, '') : '',
                    discount: row.DisAmount != null ? toNum(row.DisAmount) : 0,
