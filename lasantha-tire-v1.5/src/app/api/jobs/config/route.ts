...
// Ensure Node runtime so fs access works in all environments
export const runtime = 'nodejs';

// Point to the root configuration file shared with the Bot
// Try multiple paths to be robust against CWD differences
const CONFIG_PATHS = [
  path.join(process.cwd(), '..', 'config', 'jobs.json'), // When running from lasantha-tire-v1.0
  path.join(process.cwd(), 'config', 'jobs.json'),       // When running from root or if config is local
  path.join('c:', 'whatsapp-sql-api', 'config', 'jobs.json'), // Absolute fallback
  path.join('c:', 'whatsapp-sql-api', 'jobs-config.json') // Root fallback for real config
];
...
async function readConfigFile() {
  for (const p of CONFIG_PATHS) {
    try {
      ...
      console.log(`API: Successfully read config from ${p}`);
      return { data, path: p };
    } catch (e) {
...
export async function GET() {
  console.log('API: GET /api/jobs/config called');
  try {
    // Try to read existing config
    try {
      console.log('API: Attempting to read jobs config...');
      const { data, path: usedPath } = await readConfigFile();
      const config = JSON.parse(data);
      console.log('API: Read success. Keys:', Object.keys(config).length);
      ...
      if (Object.keys(merged).length > Object.keys(config).length) {
        console.log('API: Merging new jobs and saving...');
        await fs.writeFile(usedPath, JSON.stringify(merged, null, 2));
      }
      
      console.log('API: Returning 200 OK with', Object.keys(merged).length, 'jobs');
      return NextResponse.json(merged, { status: 200 });
    } catch (error) {
      console.error('API: File read error (creating defaults):', error);
      // If file doesn't exist, create it with defaults at the first preferred path
      const defaultPath = CONFIG_PATHS[0];
      await fs.mkdir(path.dirname(defaultPath), { recursive: true });
      await fs.writeFile(defaultPath, JSON.stringify(JOB_DEFAULTS, null, 2));
      console.log('API: Returning defaults with 200 OK');
      return NextResponse.json(JOB_DEFAULTS, { status: 200 });
    }
  } catch (error) {
    console.error('API: Critical error in GET handler:', error);
...
export async function POST(request: Request) {
  try {
    ...
    let savePath = CONFIG_PATHS[0]; // Default to first path if read fails
    ...
    try {
      ...
      // Ignore error, use defaults and default path
      console.log('API: Could not read config for POST, using defaults');
    }
    ...
    await fs.writeFile(savePath, JSON.stringify(currentConfig, null, 2));
    console.log(`API: Saved config to ${savePath}`);
