    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          {/* Controls */}
          <div className="flex flex-wrap items-center gap-2 md:gap-3 bg-slate-800/50 p-2 rounded-2xl border border-white/10 backdrop-blur-xl w-full md:w-auto">
            <button
              onClick={() => setAutoRefresh(!autoRefresh)}
              className={`flex-1 md:flex-none justify-center px-3 md:px-4 py-2 rounded-xl transition-all flex items-center gap-2 text-xs md:text-sm font-bold ${
                autoRefresh 
                  ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 shadow-lg shadow-emerald-500/10' 
                  : 'bg-slate-700/50 text-slate-400 border border-white/5 hover:bg-slate-700'
              }`}
            >
              {autoRefresh ? <Pause className="w-3 h-3 md:w-4 md:h-4" /> : <Play className="w-3 h-3 md:w-4 md:h-4" />}
              <span className="whitespace-nowrap">{autoRefresh ? 'Auto-refresh ON' : 'Auto-refresh OFF'}</span>
            </button>

            <div className="hidden md:block h-8 w-px bg-white/10 mx-1"></div>

            <div className="flex items-center gap-2 px-2 md:px-3 bg-slate-900/50 rounded-lg py-1 md:py-0 border border-white/5 md:border-none md:bg-transparent">
              <Clock className="w-3 h-3 md:w-4 md:h-4 text-slate-400" />
              <select
                value={refreshInterval}
                onChange={(e) => setRefreshInterval(Number(e.target.value))}
                className="bg-transparent text-slate-300 text-xs md:text-sm font-medium focus:outline-none cursor-pointer"
              >
            ...
            <button
              onClick={fetchLogs}
              disabled={isLoading}
              className="p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-all shadow-lg shadow-blue-600/20 disabled:opacity-50 disabled:cursor-not-allowed ml-auto md:ml-0"
            >
                ...
                {logs.map((log, index) => (
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-start gap-1 md:gap-3">
                      <div className="flex items-center gap-2 md:block min-w-[140px]">
                        <span className="text-[10px] text-slate-500 whitespace-nowrap font-medium pt-0.5">
                          {formatTimestamp(log.timestamp)}
                        </span>
                        <div className="flex md:hidden items-center gap-2">
                          <span className={`text-[10px] uppercase font-bold pt-0.5 ${
                          ...
                          <span className="text-[10px] text-slate-400 font-bold pt-0.5">
                            {log.source}
                          </span>
                        </div>
                      </div>
                      
                      <span className={`hidden md:block text-[10px] uppercase font-bold min-w-[50px] pt-0.5 ${
                        log.level === 'error' ? 'text-rose-400' : 
                        log.level === 'warn' ? 'text-amber-400' : 
                        'text-emerald-400'
                      }`}>
                      ...
                      <span className="hidden md:block text-[10px] text-slate-400 min-w-[60px] font-bold pt-0.5">
                        {log.source}
                      </span>
