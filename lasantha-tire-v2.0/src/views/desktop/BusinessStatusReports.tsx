...
import { exportToPDF, exportToExcel } from '@/core/lib/reportExport';
...
export default function BusinessStatusReports() {
  ...
  return (
    <div className="space-y-6">
      {/* Header Controls */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-800/50 backdrop-blur-xl p-4 md:p-6 rounded-2xl md:rounded-[2rem] shadow-xl border border-white/10">
        <div>
          <h2 className="text-lg font-semibold text-white">Financial Overview</h2>
          <p className="text-xs md:text-sm text-slate-400">Last updated: {new Date(data.lastUpdated).toLocaleString()}</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          <button 
            onClick={fetchData}
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-slate-700/50 hover:bg-slate-700 text-white rounded-xl text-sm transition-colors font-medium border border-white/5"
          >
            <RefreshCw size={16} />
            Refresh
          </button>
          <div className="relative group flex-1 sm:flex-none">
            <button 
              className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 rounded-xl text-sm transition-colors font-medium border border-blue-500/20"
            >
              <Download size={16} />
              Export
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-800/50 backdrop-blur-xl p-4 md:p-6 rounded-2xl md:rounded-[2rem] shadow-xl border border-white/10">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-xs md:text-sm font-medium text-slate-400">Total Receivables</p>
              <h3 className="text-xl md:text-2xl font-bold text-white mt-1">{formatCurrency(data.arAging.total)}</h3>
            </div>
        ...
        <div className="bg-slate-800/50 backdrop-blur-xl p-4 md:p-6 rounded-2xl md:rounded-[2rem] shadow-xl border border-white/10">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-xs md:text-sm font-medium text-slate-400">Total Payables</p>
              <h3 className="text-xl md:text-2xl font-bold text-white mt-1">{formatCurrency(data.apAging.total)}</h3>
            </div>
            <div className="p-2 md:p-3 bg-red-500/10 rounded-xl border border-red-500/20">
        ...
        <div className="bg-slate-800/50 backdrop-blur-xl p-4 md:p-6 rounded-2xl md:rounded-[2rem] shadow-xl border border-white/10">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-xs md:text-sm font-medium text-slate-400">Cash Position</p>
              <h3 className="text-xl md:text-2xl font-bold text-white mt-1">{formatCurrency(data.totalCash)}</h3>
            </div>
        ...
        <div className="bg-slate-800/50 backdrop-blur-xl p-4 md:p-6 rounded-2xl md:rounded-[2rem] shadow-xl border border-white/10">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-xs md:text-sm font-medium text-slate-400">Top Customer Exposure</p>
              <h3 className="text-xl md:text-2xl font-bold text-white mt-1">
                {data.topCustomers.length > 0 ? formatCurrency(data.topCustomers[0].CurrentBalance) : '0'}
              </h3>
            </div>
            <div className="p-2 md:p-3 bg-violet-500/10 rounded-xl border border-violet-500/20">
      ...
      {/* Tab Content */}
      <div className="bg-slate-800/50 backdrop-blur-xl p-4 md:p-8 rounded-2xl md:rounded-[2rem] shadow-xl border border-white/10 min-h-[400px]">
        {activeTab === 'overview' && (
