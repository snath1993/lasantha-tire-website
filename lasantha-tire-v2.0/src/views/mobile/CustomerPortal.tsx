...
export default function CustomerPortal() {
  ...
  const sendOTP = async () => {
    ...
    try {
      const response = await fetch(`${BOT_API_PROXY}/api/auth/send-otp`, {
        method: 'POST',
  ...
  const verifyOTP = async () => {
    ...
    try {
      const response = await fetch(`${BOT_API_PROXY}/api/auth/verify-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const fetchCustomerData = async (customerPhone: string) => {
    ...
    try {
      // Fetch quotations
      const quotationsRes = await fetch(`${BOT_API_PROXY}/api/quotations/customer/${customerPhone}`);
      ...
      // Fetch vehicles
      const vehiclesRes = await fetch(`${BOT_API_PROXY}/api/customers/vehicles/${customerPhone}`);
                  ...
                  <div key={quotation.Id} className="bg-slate-700/50 rounded-xl p-4">
                    ...
                    {!quotation.IsBooked && !quotation.IsExpired && (
                      <div className="flex gap-2">
                        <a
                          href={`https://book.lasanthatyre.com?ref=${quotation.QuotationNumber || quotation.RefCode}`}
                          target="_blank"
