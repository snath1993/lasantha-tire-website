...
export default function TireSearch({ onAddToQuote, quoteItems = [] }: { onAddToQuote: (item: TireProduct) => void, quoteItems?: any[] }) {
  ...
  const buildShareMessage = (
    ...
    text += itemsToShare.map((item) => {
      ...
      // Only clean up size from description for Tyres (non-service items)
      if (!isService && width && rim) {
      ...
      // For services, show only Description and Price (No header label)
      if (isService) {
        return `🏷️ ${desc}\n💰 ${priceDisplay}/=`;
      }
      ...
      <div className="w-full h-full bg-slate-900 flex flex-col">
        ...
        <div className="flex flex-col justify-end p-6 pb-3 relative" style={{height: '35vh'}}>
            ...
            <div className="absolute right-6 top-6 flex items-center gap-3 z-50">
                {/* AI Manager Button */}
                <button
                    onClick={() => router.push('/dashboard?tab=titan')}
                    className="p-3 bg-slate-800/50 backdrop-blur-md rounded-xl border border-white/10 text-cyan-400 hover:bg-cyan-600 hover:text-white transition-all shadow-lg shadow-cyan-900/20"
                >
                ...
                <button
                    ...
                    className="p-3 bg-slate-800/50 backdrop-blur-md rounded-xl border border-white/10 text-violet-400 hover:bg-violet-600 hover:text-white transition-all"
                >
                         ...
                         <div className="flex flex-col items-end">
                                            ...
                                            if (e.key === 'Enter') {
                                                ...
                                                if (!isNaN(val)) {
                                                    if (isService) {
                                                        ...
                                                        setCustomItemPrices(prev => ({ ...prev, [item.ItemId]: val }));
                                                    }
                                                }
                            ...
                            <p className="text-[10px] text-slate-500 mt-0.5">
                                {isService ? (
                                    ...
                                    customItemPrices[item.ItemId] !== undefined ? 'Custom' :
                            ...
                            {/* Reset Button */}
                            {(customItemPrices[item.ItemId] !== undefined || customServicePrices[item.ItemId] !== undefined) && (
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        if (isService) {
                                    ...
                                    className="text-[10px] text-slate-400 flex items-center gap-1 hover:text-white mt-1"
                                >
